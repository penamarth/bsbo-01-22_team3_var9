...
== Альтернативный сценарий 1: Обработка платежа ==
DM -> PM : processPayment(applicationID, amount)
activate PM
...
== Альтернативный сценарий 3: Ошибка отправки уведомления ==
DM -> NM : sendNotification(Notification)
activate NM
...
== Альтернативный сценарий 4: Ошибка обновления статуса заявки ==
DM -> AM : updateApplication(applicationID, updatedApplication)
activate AM
...
== Расширения ==
== Расширение 1: Массовая выдача грантов ==
FH -> DM : approveMultipleApplications(applicationsList)
activate DM
DM -> PM : processPayment(applicationID, amount) (for each application)
activate PM
PM --> DM : Payments processed
deactivate PM
DM -> NM : sendNotification(Notification) (for each applicant)
activate NM
NM --> AP : Notifications sent
deactivate NM
DM --> FH : Batch approval completed
deactivate DM

== Расширение 2: Интеграция с внешними системами ==
FH -> PM : initiatePaymentWithExternalSystem(applicationID, amount)
activate PM
PM -> PM : Connect to external payment gateway
PM --> FH : Payment processed via external system
deactivate PM
